...
export default function CompleteTeacherProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!teacherKey.trim()) {
      return toast.error("Teacher registration key is required");
    }

    setLoading(true);

    try {
      const res = await api.post("/auth/complete-teacher-profile", {
      ...
      toast.success("Registration key verified successfully!");
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/60 p-10 border border-slate-100">
          <div className="flex justify-center mb-8">
            <div className="w-20 h-20 bg-slate-900 rounded-3xl flex items-center justify-center text-white shadow-xl shadow-slate-200 rotate-3 hover:rotate-0 transition-transform duration-300">
              <ShieldCheck size={40} />
            </div>
          </div>
          ...
          <div className="text-center mb-10">
            <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-2">
              Verify Access <span className="text-indigo-600">.</span>
            </h1>
            <p className="text-slate-500 font-medium">
              Enter your Teacher Registration Key to finish setting up your account.
            </p>
          </div>
